...
## Nog te doen (bewust als placeholder achtergelaten)
...
- **Contactformulier** (`components/Contact.tsx`): `handleSubmit` is een
  stub. Koppel aan een server action, API-route, of formulierdienst
  (bijv. Resend, Formspree) om echte aanvragen te verwerken.
...
- **Lighthouse-score**: is niet lokaal gemeten omdat dit geen volledig
  draaiend project is (geen `node_modules`/build hier uitgevoerd) — test dit
  na integratie in je bestaande project.
