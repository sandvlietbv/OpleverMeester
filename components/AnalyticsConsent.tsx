...
export default function AnalyticsConsent(){
  ...
  useEffect(()=>{const stored=window.localStorage.getItem(CONSENT_KEY);const initial:Consent=stored==="accepted"||stored==="declined"?stored:null;setConsent(initial);setLoaded(true);if(initial==="accepted")startAnalytics();else stopAnalytics()},[]);
  ...
  if(consent&&!preferencesOpen)return <button type="button" onClick={()=>setPreferencesOpen(true)} className="fixed bottom-3 left-3 z-[70] rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-700 shadow-md hover:bg-slate-50">Cookievoorkeuren</button>;
